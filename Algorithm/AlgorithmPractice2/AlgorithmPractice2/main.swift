...
extension String {
    func isThreeSix() -> Bool {
        var str = self
        while true {
            guard let firstIndex = str.firstIndex(of: "6") else { break }
            guard firstIndex != str.index(str.endIndex, offsetBy: -2) &&
                    firstIndex != str.index(str.endIndex, offsetBy: -1) else { break }
            ...
            list.compactMap { $0 }
            guard list.count == 3 else { break }
            if list.map({ String($0) }).reduce("", +) != "666" {
